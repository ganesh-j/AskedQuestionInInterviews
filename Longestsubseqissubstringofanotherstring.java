...
public class GFG
{
                                ...
                                int n, int m)
    {
        ...
        for (int i = 1; i <= m; i++) { //row is having length of string y, longest subsequence of x should be 
        //be present as substring in y
            for (int j = 1; j <= n; j++) {
                ...
                // row i.e dp[i-1][j-1] as we need to substring from [0 to i] into subsequence of [0 to j-1]
                else
                    dp[i][j] = dp[i][j - 1];
            }
        }
